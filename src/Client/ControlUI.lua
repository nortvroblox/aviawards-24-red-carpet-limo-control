...
local FuzzySearch = require(ReplicatedStorage.LimoCommon.FuzzySearch)
local Iris = require(ReplicatedStorage.LimoPackages.iris)

local limoAnimationProgress = Iris.State()
local limoState = Iris.State()
local limoCurrentPlayers = Iris.State({})

return function(props)
	return function()
		local searchingPlayer = Iris.State("")
		local selectedPlayers = Iris.State({})

		limoState:set(props.getLimoState and props.getLimoState() or "Stopped")
		limoAnimationProgress:set(props.getLimoAnimationProgress and props.getLimoAnimationProgress() or 0)
		limoCurrentPlayers:set(props.getLimoCurrentPlayers and props.getLimoCurrentPlayers() or {})

		Iris.Window({
			[Iris.Args.Window.Title] = "Limo Control UI",
		}, { size = Iris.State(Vector2.new(800, 500)) })

		Iris.Text({ "Limo Control UI" })
		Iris.Separator()
		Iris.Text({ `Current Limo State: {limoState:get()}` })
		Iris.Text({ "Current Players:" })
		Iris.Table({ 1 })
		do
			for _, player in pairs(limoCurrentPlayers:get()) do
				Iris.NextColumn()
				Iris.Text({ player.Name })
			end
		end
		Iris.End()
		Iris.ProgressBar({ "Animation Progress" }, { progress = limoAnimationProgress })
		Iris.Separator()

		Iris.CollapsingHeader({ "Limo Enter Sequence" })
		do
			Iris.Table({ 2 })
			Iris.NextColumn()
			Iris.Text({ "Search player to enter sequence" })
			Iris.InputText({ "Player Name" }, { text = searchingPlayer })
			Iris.Text({ "Search results:" })
			Iris.Table({ 2 })
			do
				local playerNames = {}
				local playerNameToPlayer = {}
				if #searchingPlayer:get() > 1 then
					for _, player in pairs(Players:GetPlayers()) do
						local lowercaseName = string.lower(player.Name)
						local lowercaseDIsplayName = string.lower(player.DisplayName)
						table.insert(playerNames, lowercaseName)
						playerNameToPlayer[lowercaseName] = player

						table.insert(playerNames, lowercaseDIsplayName)
						playerNameToPlayer[lowercaseDIsplayName] = player
					end
				end
				local sortedByDistance = FuzzySearch.SortByDistance(string.lower(searchingPlayer:get()), playerNames, 5)
				--// limit to 10
				local limitedList = {}
				for i = 1, math.min(#sortedByDistance, 10) do
					limitedList[playerNameToPlayer[sortedByDistance[i]]] = true
				end

				--// display the list
				for player, _ in pairs(limitedList) do
					Iris.NextColumn()
					Iris.Text({ player.Name })
					Iris.NextColumn()
					if Iris.Button({ "Select" }).clicked() then
						local newSelectedPlayers = selectedPlayers:get()
						newSelectedPlayers[player] = true
						selectedPlayers:set(newSelectedPlayers)
					end
				end
				Iris.End()
			end
			Iris.NextColumn()
			Iris.Text({ "Selected player: " })
			Iris.Table({ 2 })
			do
				for selectedPlayer, _ in pairs(selectedPlayers:get()) do
					Iris.NextColumn()
					Iris.Text({ selectedPlayer.Name })
					Iris.NextColumn()
					-- Iris.Button({ "Remove Player" }).clicked(function()
					if Iris.Button({ "Remove Player" }).clicked() then
						print("selectedPlayers", selectedPlayers:get())
						local newSelectedPlayers = selectedPlayers:get()
						for player, _ in pairs(selectedPlayers:get()) do
							newSelectedPlayers[player] = nil
						end
						selectedPlayers:set(newSelectedPlayers)
					end
				end
			...
			Iris.End()
			Iris.End()
			if Iris.Button({ "Start/Restart Enter Sequence" }).clicked() then
				local players = {}
				for player, _ in pairs(selectedPlayers:get()) do
					table.insert(players, player)
				end
				props.startEnterSequence(players)
				selectedPlayers:set({})
			end
		end
		Iris.End()
		Iris.Separator()
		Iris.CollapsingHeader({ "Limo Exit Sequence" })
		do
			if Iris.Button({ "Start/Restart Exit Sequence" }).clicked() then
				props.startExitSequence()
			end
		end
		Iris.End()
